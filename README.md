...
このリポジトリはCloud VPSの中に存在しています。独自ドメイン`yuasys.org`から派生させたサブドメイン`nob.yuasys.org`をベースにしています。

1. サーバーの絶対パスは、`/home/nob/work/docker-web-server-01-nginx`です。このパス名はこのディレクトリの目的や位置づけを推測しやすいように名付けました。

    - /home/nob: 所有者は`nob`というrootではないユーザーです
